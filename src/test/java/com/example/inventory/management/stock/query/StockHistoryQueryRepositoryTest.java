...
class StockHistoryQueryRepositoryTest extends AbstractIntegrationTest {
    ...
    void 생성시각_역순으로_정렬된다() {
        ...
        PageResponse<StockHistoryResponse> page = stockHistoryQueryRepository.findByProductId(productId, 0, 10);

        assertThat(page.content()).extracting(StockHistoryResponse::id).containsExactly(newestId, middleId, oldestId);
    ...
    void 생성시각이_같으면_id_역순으로_정렬된다() {
        ...
        assertThat(page.content()).extracting(StockHistoryResponse::id).containsExactly(third, second, first, older);
    ...
    void 페이지를_나누어_조회하고_다음_페이지_여부를_반환한다() {
        ...
        PageResponse<StockHistoryResponse> first = stockHistoryQueryRepository.findByProductId(productId, 0, 2);
        PageResponse<StockHistoryResponse> second = stockHistoryQueryRepository.findByProductId(productId, 1, 2);
        PageResponse<StockHistoryResponse> last = stockHistoryQueryRepository.findByProductId(productId, 2, 2);
        PageResponse<StockHistoryResponse> beyond = stockHistoryQueryRepository.findByProductId(productId, 3, 2);

        assertThat(first.content()).extracting(StockHistoryResponse::id).containsExactly(ids[4], ids[3]);
        assertThat(first.hasNext()).isTrue();
        assertThat(second.content()).extracting(StockHistoryResponse::id).containsExactly(ids[2], ids[1]);
        assertThat(second.hasNext()).isTrue();
        assertThat(last.content()).extracting(StockHistoryResponse::id).containsExactly(ids[0]);
    ...
    void 다른_상품의_이력은_포함하지_않는다() {
        ...
        assertThat(page.content()).extracting(StockHistoryResponse::id).containsExactly(mine);
    }
