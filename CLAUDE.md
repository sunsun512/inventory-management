...
- **Persistence**: schema is owned by Flyway migrations in `src/main/resources/db/migration` (Hibernate `ddl-auto: validate`). Stock quantity lives on `product`; every change is recorded in `stock_history`. `src/main/resources/data.sql` seeds local data only when tables are empty.
